...
**初期コミット 1 つから始まっている。** 雛形 `myapp` から派生したあと、git 履歴を持ち込まずに作り直した(雛形の履歴には旧案件の秘密情報が残っているため)。**ここより前を `git log` で遡ることはできない**ので、設計判断の経緯は `docs/decisions/` の ADR に、踏んだ罠は `docs/agent-traps.md` にある。

**docs には雛形(`myapp`)としての視点の文章がまだ残っている。** 「この雛形は」「案件で使うときは」という書き方を見つけたら、それは**案件視点へ書き換える対象**であって現行の方針ではない(`docs/known-issues.md` に積み残しとして記録してある)。**内容が間違っているわけではない**ので、書き換えは見つけたときでよい。
