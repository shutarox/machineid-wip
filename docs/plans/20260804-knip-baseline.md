...
| `pgdump` 系 | 1-5 で「全削除」と決定 | **残存していた。しかも壊れていた**(下記) |
| `uuid` / `date-fns` / `jotai-cache` / `tsconfig-paths` | 1-16 の依存掃除の趣旨 | 同じく 3e-3 まで残存 |

**`pgdump` はその後「残す」に決定が覆った**(2026-08-04・ユーザー判断。経緯は
`docs/template-repo-workplan.md` の 1-5 と「`pgdump/pgdump.sh` の残存」の項)。
ただし**この事例の教訓は消えていない** — むしろ強まっている。残存に気づいて動作確認したところ、
**`pg_dump` はクライアント 17 / サーバ 18 の不一致で中断しており、しかもスクリプトが失敗を無視して
「Backup saved」と表示していた**(0 バイトのバックアップが成功に見える)。
**使われないまま置かれたものは、残っているだけでなく腐る。** 現在は修理済みで、
手順は `pgdump/README.md`。
