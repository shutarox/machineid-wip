...
作成: 2026-08-06
状態: **進行中**(1 号機 = `machineid`。手順 1〜5 は完了。**次は手順 6 = まっさら化**)
...
| 3. 削ぎ落とし | **完了(2026-08-07)**。`terraform.example/` と雛形化の作業記録を削除(-6,877 行)。受け入れテストの記録は `docs/agent-traps.md` へ、S3 アップロードの判断は ADR へ移した。**手順 6 と同時にやる残件が 2 つ**(下の削ぎ落とし表を参照): `deploy/deploy_prod-main.sh` の clone 先の切り替えと、C(まっさら化後) |
| 4. 痕跡の検査 | **3 巡してクリア(2026-08-07)**。まっさら化の直前に最終確認としてもう一度回す |
| 5. 検証 | **完了(2026-08-07)**。削ぎ落とし後のツリーで CI 緑(lint / check / test / e2e の 4 ジョブ)。`pnpm verify` 緑・`pnpm knip` ベースラインどおり。**CI の作り直しは不要だった**(下記) |
...
| D | 痕跡・秘密情報の検査(手順 4) | **3 巡してクリア(2026-08-07)**。秘密情報・個人情報 0 件、リンク切れ 0 件、残る参照はすべて意図的。→ 手順 4 の「実施記録」。**まっさら化の直前にもう一度回す**(手順 5 の CI 作り直しでファイルが動くため) |

**A〜D はすべて決着した**(A 完了 / B 見送り / C はまっさら化の後へ移動 / D は 3 巡してクリア)。**次は手順 5(検証)**。
...
**1 巡目**(手順 3 の削除より前)を下記に、**2 巡目・3 巡目**をその後に記す。
方針どおり実施した結果、**検索語に依存しない検出が効き、案件名の grep では出ないものが出た。**
...
##### 2 巡目(参照の張り替え後)— **検出器の欠陥が 1 つ見つかった**

手順 3 で `template-repo-workplan.md` を消した直後に回したところ、**その参照 6 件を見逃した**。
原因は「存在しない参照先」の検出が `glob('**/<path>')` で救済していたことで、
**`build/`(デプロイ用の clone)に残る古いコピーに一致していた**。

**教訓: 実在確認は `git ls-files` の集合に対して行う。** 作業ディレクトリを舐めると、
ビルド成果物・キャッシュ・`node_modules` が「実在する」という誤った答えを返す。
直したうえで再実行し、残った参照を張り替えた。

あわせて **`docs/template-feedback.md`(まっさら化後も残る)から旧案件の実値を伏せた** —
実 AWS アカウント ID・CloudFront ディストリビューション ID・実ドメインが入っていた。
指摘の内容は値なしで成立するため、記述だけ残した。

##### 3 巡目(手順 3 完了後)— **クリア**

| 検出 | 結果 |
|---|---|
| 資格情報・秘密鍵・webhook | **0 件** |
| 個人情報 | **0 件**(`0000-7000-8000` は UUID の一部、`最新仕様` は人名正規表現の誤検出) |
| markdown リンク切れ | **0 件** |
| 存在しない参照先 | 20 種。**すべて意図的**(「存在しないこと」が指摘の中身 / これから作るもの / 削除の記録 / `myapp` 側のファイル / 意図的に追跡しないもの) |
| ドメイン形式 | 旧案件のドメインは 2 箇所のみ。**この計画ファイル自身**(検出の記録。まっさら化で消える)と `template-feedback.md` の `myappdomain.com`(`myapp` 側のプレースホルダを指す正しい記述) |
| 12 桁の数値 | `439996178164` / `394530725171` は**いずれも自分のアカウント**。`000000000000` はプレースホルダ |
| 案件名の grep(補助) | この計画ファイルのみ |

**まっさら化の直前にもう一度回すこと。** 手順 5 の CI 作り直しでファイルが動くため。
そのときは上の 3 巡目と同じ結果になるはずで、**差分が出たら手順 5 で何かを持ち込んだということ**。

### 5. 検証

`pnpm verify` 緑 + CI 緑 + `pnpm knip` ベースライン + 手順 4 の検査がすべて空であること。

**当初あった「CI を作り直す」は不要だった**(2026-08-07 実測)。
「secrets・OIDC・ECR 名が案件のものに変わるため」と書いていたが、**`ci.yml` は
GitHub secrets も OIDC も ECR も使っていない** — postgres / MinIO の service コンテナと
ダミーの SSM キャッシュだけで完結しており、外部依存がゼロ。**新リポジトリに workflow ファイルごと
コミットが移ればそのまま動く**ので、作り直す対象が存在しない。

したがって手順 5 の実体は「**削ぎ落とし後のツリーで CI を 1 回通す**」だけになる。
ローカルの `pnpm verify` が見ないもの(`--frozen-lockfile` での install、
**Prisma 生成物と OpenAPI 型のドリフト検査**)がここで効く。
...
- `deploy/deploy_prod-main.sh` の clone 先を `<project>-wip` → `<project>` に変える
  (**先に変えるとまっさら化までデプロイが壊れる**ので、このタイミングでしかできない)
- **この計画ファイルを消す前に、次の派生で効く知見を `docs/template-feedback.md` へ移す**
  (実施済み: 項目 19〜24。**移さないと計画ごと失われる**)
- C(ドキュメントの案件化)の申し送り(本体は `docs/known-issues.md` に置いてある)
