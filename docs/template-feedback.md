...
| 20 | **「存在しない参照先の検出」は `git ls-files` の集合に対して行う。** 1 号機では `glob('**/<path>')` で救済していたため、**`build/`(デプロイ用の clone)に残る古いコピーに一致して 6 件を見逃した**。作業ディレクトリを舐めると、ビルド成果物・キャッシュ・`node_modules` が「実在する」という誤った答えを返す。**この検出はドメイン総なめと並んで最も当たりが多い**(1 号機では `hcho.jp` もこれで見つかった)ので、精度を落とさないこと |
