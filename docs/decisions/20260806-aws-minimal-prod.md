...
|---|---|
| 2026-08-06 | 初版 |
| 2026-08-07 | **DB 接続の記述を改定(2 段階)。** ①当初は「タスク定義に RDS のエンドポイントを直接書かず、プライベートホストゾーンの `db-pg.<local_domain_name>` 経由で参照する」としていたが、**実機で成立しなかった**。`rds.force_ssl` の既定が `1`(Aurora / RDS とも)で SSL が必須である一方、**証明書の SAN は RDS の実エンドポイントだけ**なので CNAME 経由ではホスト名検証を通せない(`sslmode=verify-ca` でも pg の実装では検証される)。いったん「`DB_URL` に実エンドポイント + `verify-full`」に変更した。②その後、**エンドポイントが変わったときの作業量を比較して CNAME + `sslmode=no-verify` に戻した**。エンドポイントが変わるのは実質「スナップショット / PITR からの復元」= **障害復旧時**で、実エンドポイント方式だと復旧の最中に **SSM の書き換えとタスクの強制入れ替えという手作業が 2 つ増える**。CNAME 方式なら terraform apply が CNAME を向け直し、**アプリは接続の張り直しで自力回復する**。失うのはホスト名検証だが、RDS は private サブネットで SG は `is_app` からの 5432 のみ。**この経路が破られる状況では既にタスク自体が取られている可能性が高い** |
| 2026-08-07 | **`DB_URL` を SSM から外した。** 秘密なのはパスワードだけで、ホスト(CNAME)・ユーザ・DB 名・接続パラメータはすべて静的だから。2 本(`DB_PASSWORD` と `DB_URL`)持つと**ローテーションのたびに両方を更新することになり、片方だけ直して壊す**。接続文字列は entrypoint(`etc/onstart-prod-main.sh`)が `DB_PASSWORD` と `APPX_DB_*` から組み立てる |
...
**削らなかったもの: 監視**(2026-08-07 追加。`main-app/38_monitoring.tf`)。
アプリのエラーログと「ALB に健全なホストがゼロ」「ECS タスクの起動失敗」を Slack へ流す
Lambda 2 つを置いている。**常時起動ではなくイベント駆動なので、この構成でも実質無料**
(月数十回の起動)。**ここを削ると、上の 1〜5 で受け入れたリスクに誰も気づけなくなる** —
Single-AZ の障害も、public サブネットでの異常も、検知手段はこれしかない。
...
そのうえで、より本質的な問題は分割の粒度ではなく**長期有効な静的アクセスキーを持つ IAM ユーザーを terraform で作っていること**である。このリポジトリには実際に、静的な認証情報(SES SMTP)がコミット履歴に混入しローテーションが必要になった事例がある(`docs/template-feedback.md` の項目 25)。**存在しない鍵は漏れない。**
...
- **DB への接続はプライベートホストゾーンの CNAME(`db-pg.<local_domain_name>`)経由で、`sslmode=no-verify`**(2026-08-07 改定。下記「改定履歴」)。**実エンドポイントを書かないのは、障害復旧時の手作業を減らすため** — スナップショット / PITR から復元するとエンドポイントが変わるが、CNAME なら terraform apply が向け直し、アプリは接続の張り直しで自力回復する。`verify-full` にすると CNAME ではホスト名検証が通らず、実エンドポイントを書く必要が生じ、復旧のたびに SSM 更新とタスク入れ替えが要る
- **`DB_URL` を SSM に持たないこと。** entrypoint が `DB_PASSWORD` と `APPX_DB_{HOST,USER,NAME,PARAMS}` から組み立てる。**接続文字列を SSM に置くと、パスワードのローテーションで 2 箇所を直すことになる**
