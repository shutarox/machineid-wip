...
## 対象は**ローカル DB のみ**。本番には使えない

本番の RDS に対しては、**定期実行になっていないという以前に手動でも実行できない**。

1. 開発コンテナから RDS に**届かない**(private サブネット・SG は ECS タスクからのみ)
2. ECS Exec でコンテナに入れば `pg_dump` は動くが、**出力を持ち出せない**
   (本番イメージに `aws` も `curl` も無い。意図的に slim にしてある)

本番のバックアップは **RDS の自動バックアップ(保持 7 日・PITR)だけ**。
論理バックアップを取れるようにする方針は `docs/known-issues.md`
「本番の論理バックアップが取れない」に記録してある。
